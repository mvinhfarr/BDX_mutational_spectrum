...
def load_summary_dir(snv_dir):
    chr_singletons = []

    for f_name in os.listdir(snv_dir):
        path = os.path.join(snv_dir, f_name)
        df = pd.read_csv(path, header=0, index_col=None)
        chr_singletons.append(df)

    raw_mut_spec = pd.concat(chr_singletons, axis=0)
    return raw_mut_spec


def load_formatted_summary(snv_file):
    raw_snv_data = pd.read_csv(snv_file, header=0, index_col=None)
    raw_snv_data.set_index()
